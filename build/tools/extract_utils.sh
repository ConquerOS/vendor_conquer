...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$CONQUER_ROOT"/prebuilts/tools-conquer/common/smali/baksmali.jar
        export SMALIJAR="$CONQUER_ROOT"/prebuilts/tools-conquer/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$CONQUER_ROOT"/prebuilts/tools-conquer/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$CONQUER_ROOT"/prebuilts/tools-conquer/${HOST}-x86/bin/compact_dex_converter
    fi

    if [ -z "$PATCHELF" ]; then
        export PATCHELF="$CONQUER_ROOT"/prebuilts/tools-conquer/${HOST}-x86/bin/patchelf
    fi
    ...
    if [ ! -f "$CONQUER_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$CONQUER_TARGET" >/dev/null; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$CONQUER_ROOT"/vendor/conquer/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
