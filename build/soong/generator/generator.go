...
func init() {
	android.RegisterModuleType("conquer_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = conquerExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = conquerExpandVariables(ctx, dep_file)
	...
	cmd := conquerExpandVariables(ctx, String(g.properties.Cmd))
